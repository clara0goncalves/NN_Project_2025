...
class ModelEvaluator:
    def __init__(self, model_path, config):
        ...
        # 2. Check the config to see if the model was compiled during training
        if config.get('use_torch_compile', False) and hasattr(torch, 'compile'):
            print("Compiling model for evaluation to match saved state...")
            self.model = torch.compile(self.model)
        
        # 3. Now, load the state dictionary into the (potentially compiled) model
    ...
    def evaluate_dataset(self, dataloader, num_examples=5):
        # This function does not need changes, but it's here for completeness.
        all_metrics = {'dice':[], 'iou':[], 'pixel_acc':[], 'precision':[], 'recall':[], 'f1':[]}
        all_cm = {'TP': 0, 'TN': 0, 'FP': 0, 'FN': 0}
        
        print("Evaluating model on the test set...")
        with torch.no_grad():
            for i, (images, masks, ignore_masks) in enumerate(tqdm(dataloader, desc="Evaluating")):
                images, masks, ignore_masks = images.to(self.device), masks.to(self.device), ignore_masks.to(self.device)
                outputs = self.model(images)
                
                for j in range(images.size(0)):
                    output_j = outputs[j].unsqueeze(0)
                    mask_j = masks[j]
                    pred_j = (torch.sigmoid(output_j) > 0.5).float()
                    
                    valid_pixels = ignore_masks[j].view(-1)
                    pred_flat = pred_j.view(-1)[valid_pixels]
                    mask_flat = mask_j.view(-1)[valid_pixels]

                    if mask_flat.numel() > 0: # Avoid division by zero on empty masks
                        all_metrics['dice'].append(dice_score(pred_flat, mask_flat).item())
                        all_metrics['iou'].append(iou_score(pred_flat, mask_flat).item())
                        all_metrics['pixel_acc'].append(pixel_accuracy(pred_flat, mask_flat).item())
                        precision, recall, f1 = precision_recall_f1(pred_flat, mask_flat)
                        all_metrics['precision'].append(precision.item())
                        all_metrics['recall'].append(recall.item())
                        all_metrics['f1'].append(f1.item())
                        cm = confusion_matrix_metrics(pred_flat, mask_flat)
                        for key in all_cm: all_cm[key] += cm[key]
                    
                    if i * dataloader.batch_size + j < num_examples:
                        self.save_prediction_example(images[j], mask_j, pred_j, all_metrics['dice'][-1], i * dataloader.batch_size + j)
        
        self.log_results(all_metrics, all_cm)

    def save_prediction_example(self, image_tensor, mask_tensor, pred_tensor, dice, idx):
        # This function does not need changes, but it's here for completeness.
        ...
        ax2.imshow(mask, cmap='gray'); ax2.set_title("Ground Truth Mask"); ax2.axis('off')
        ax3.imshow(pred, cmap='gray'); ax3.set_title(f"Model Prediction (Dice: {dice:.3f})"); ax3.axis('off')
        plt.tight_layout()
    ...
    def log_results(self, metrics_dict, cm_dict):
        # This function does not need changes, but it's here for completeness.
        print("\n" + "="*30); print("--- Final Evaluation Metrics ---"); print("="*30)
        for name, values in metrics_dict.items():
            if values: # Check if list is not empty
                mean_val, std_val = np.mean(values), np.std(values)
                print(f"{name.replace('_', ' ').title():<15}: Mean={mean_val:.4f}, Std={std_val:.4f}")
        print("\n--- Overall Confusion Matrix ---")
        for key, value in cm_dict.items(): print(f"{key:<15}: {int(value)}")
        cm_array = np.array([[cm_dict['TN'], cm_dict['FP']], [cm_dict['FN'], cm_dict['TP']]])
        plt.figure(figsize=(8, 6))
        sns.heatmap(cm_array, annot=True, fmt='.0f', cmap='Blues', xticklabels=['Not Water', 'Water'], yticklabels=['Not Water', 'Water'])
