...
def save_prediction_example(image, true_mask, pred_mask, prob_mask, dice, iou, idx, save_dir):
    ...
    axes[2].imshow(pred_mask_np, cmap='gray'); axes[2].set_title(f'Predicted Mask\nIoU: {iou:.4f} | Dice: {dice:.4f}'); axes[2].axis('off')
    im = axes[3].imshow(prob_mask_np, cmap='viridis', vmin=0, vmax=1); axes[3].set_title(f'Probability Map'); axes[3].axis('off')
...
def log_and_plot_results(metrics_dict, cm_totals, save_dir):
    ...
    for i, (name, values) in enumerate(metrics_dict.items()):
        if values: sns.histplot(values, kde=True, ax=axes[i], bins=20); axes[i].set_title(f"Distribution of {name.replace('_', ' ').capitalize()}"); axes[i].set_xlabel("Score"); axes[i].set_ylabel("Frequency")
...
class ModelEvaluator:
    ...
    def evaluate_dataset(self, dataloader, num_examples=5, use_crf=False, use_tta=False):
        ...
        low_iou_dir = os.path.join(eval_dir, 'low_iou_predictions'); os.makedirs(low_iou_dir, exist_ok=True)
            ...
            for i, (images, masks) in enumerate(tqdm(dataloader, desc="Evaluating")):
                ...
                for j in range(images.size(0)):
                    ...
                    if use_crf:
                        ...
                        pred_np = apply_crf(original_image_np, prob_np); pred_j = torch.from_numpy(pred_np).float()
                    
                    # --- FIX: Ensure all metrics are calculated and stored ---
                    ...
                    if current_iou < 0.8: save_prediction_example(images[j], masks[j], pred_j, prob_j, current_dice, current_iou, f"low_iou_example_{i*dataloader.batch_size+j}", low_iou_dir)
                    if i * dataloader.batch_size + j < num_examples: save_prediction_example(images[j], masks[j], pred_j, prob_j, current_dice, current_iou, i * dataloader.batch_size + j, eval_dir)
...
class EnsembleEvaluator:
    ...
    def evaluate_dataset(self, dataloader, num_examples=5, use_crf=False, use_tta=False):
        ...
        os.makedirs(eval_dir, exist_ok=True)
        low_iou_dir = os.path.join(eval_dir, 'low_iou_predictions'); os.makedirs(low_iou_dir, exist_ok=True)
        print(f"\nEvaluating Ensemble... (TTA: {'Enabled' if use_tta else 'Disabled'}, CRF: {'Enabled' if use_crf else 'Disabled'})")
        print(f"Results will be saved to '{eval_dir}'")
        ...
        with torch.no_grad():
            for i, (images, masks) in enumerate(tqdm(dataloader, desc="Evaluating Ensemble")):
                ...
                for j in range(images.size(0)):
                    ...
                    if current_iou < 0.5: save_prediction_example(images[j], masks[j], pred_j, prob_j, current_dice, current_iou, f"low_iou_example_{i*dataloader.batch_size+j}", low_iou_dir)
                    if i * dataloader.batch_size + j < num_examples: save_prediction_example(images[j], masks[j], pred_j, prob_j, current_dice, current_iou, i * dataloader.batch_size + j, eval_dir)
        log_and_plot_results(all_metrics, cm_totals, eval_dir)
...
def main():
    ...
    if args.path:
        ...
        try:
            ...
            # You would need to add logic here to get the test_loader
    ...
    try:
        ...
        evaluator = None  # Initialize evaluator to None
        ...
        # Proceed only if an evaluator was successfully created
        if evaluator:
            ...
            _, _, test_loader = get_data_loaders(batch_size=16) # Or use a configurable batch size
